...
if __name__ == '__main__':
    ...
    np.savetxt("camera_params.txt", camera_params, delimiter=", ")
    ...
    np.savetxt("noisy_camera_params.txt", noisy_params, delimiter=", ")
    
    # Save GT markers
    markers = np.array(markers).reshape(-1, 3)
    np.savetxt("gt_markers.txt", markers, delimiter=", ")
    
    # Save Noisy markers. 
    # For xy-axis, Add small noise
    # For z-axis, Add noise to only slope
    noisy_markers = np.copy(markers)
    noisy_markers[:, :2] += np.random.normal(0, 0.05, markers[:, :2].shape)
    noisy_markers[8:, 2] += np.random.normal(0, 0.1, markers[8:, 2].shape)
    np.savetxt("noisy_markers.txt", noisy_markers, delimiter=", ")
    ...
    with open("markers_images.json", "w") as f:
